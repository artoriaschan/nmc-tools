...
type UserAgentType = 'mobile' | 'desktop'
type UserAgentList = {
  [key in UserAgentType]: string[]
}
const userAgentList: UserAgentList = {
  mobile: [
...
const getUserAgents = (ua: UserAgentType | string | 'all') => {
  let realUserAgentList =
    userAgentList[ua as UserAgentType] ??
    userAgentList.mobile.concat(userAgentList.desktop)

  return ['mobile', 'desktop', 'all'].indexOf(ua) > -1
...
export function fetch(
  ...
  return new Promise((resolve, reject) => {
    let headers: Record<string, string> = {
      'User-Agent': getUserAgents(options.ua)
    }
